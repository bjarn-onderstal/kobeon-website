...
- [x] **V9. Meetings via HubSpot i.p.v. Calendly** — Vervang de Calendly-placeholder en alle Calendly-verwijzingen door de **HubSpot Meetings-embed** (zie `docs/content-werken-bij.md` § Meetings via HubSpot). Maak een `HubspotMeetings`-component: een `<div class="meetings-iframe-container" data-src="https://meetings-eu1.hubspot.com/sjoerd-beljon/discovery-meeting?embed=true">` + het script via `next/script` (`strategy="afterInteractive"`). Gebruik 'm op /discovery-sessie (hoofd-embed) en bij de Discovery-CTA's; vervang "via Calendly" door "online, direct in je agenda".
  - **Voorbereidingsblok op /discovery-sessie** ("Wat bereid je voor?") + CTA "Mail je grootste pijnpunten alvast naar sjoerd.beljon@kobeon.nl" — copy in `docs/content-werken-bij.md` § Discovery-sessie — voorbereiding & warming-up.
  - **Pijnpunten vooraf:** voeg intake-vragen toe aan de HubSpot-meeting (grootste uitdaging, systemen, doel). De warming-up-mail + agenda-omschrijving staan in hetzelfde doc (in HubSpot in te richten, niet in de site-code).
  Klaar wanneer: de HubSpot-boekingsembed werkt op /discovery-sessie met het voorbereidingsblok, geen Calendly meer aanwezig, `npm run build` slaagt.

- [x] **V10. Echte case-assets wiren (HomeZero-video + EPSA-screenshots)** — zie `docs/content-subpaginas.md` § Cases — echte beelden & herziene teksten.
  - **HomeZero (klaar):** `/public/visuals/case-homezero.mp4` + `.webm` + `case-homezero-poster.jpg` staan al klaar (gecomprimeerd: 0,6–0,9 MB). Toon als loopende video (`autoplay muted loop playsinline`, poster, lazy-load) in een light `DeviceFrame` op de HomeZero-casepagina; overweeg 'm ook als `hero-primary`/feature-tegel op de homepage. Herzie de HomeZero-case-tekst (rijker — verduurzamingsplatform: woninggegevens, berekeningen, installateurs, afspraken).
  - **EPSA:** zodra `/public/screens/epsa-onboarding.png` + `epsa-opdrachten-mobiel.png` zijn toegevoegd: toon in browser- + phone-`DeviceFrame` op de EPSA-casepagina en op de Staffing-sector + tegels App/Portalen. Herzie de EPSA-case naar staffing/detachering (onboarding + opdrachten + uren + matching). Tot de bestanden er zijn: MiniMock-fallback.
  - **Data:** voeg `video`/`images[]` toe aan de `projects`-records; render echte media in `DeviceFrame`, anders MiniMock. Werk `public/screens.config.json` bij (homezero = video, epsa = screenshots; `status:"live"`, `anonymized:true`).
  - **Toestemming:** alleen tonen met akkoord van HomeZero/EPSA.
  Klaar wanneer: HomeZero-video draait op de case- (en evt. homepage-)plek, EPSA-screenshots op case + staffing/tegels (of nette fallback), case-teksten herzien, `npm run build` slaagt.
...
- [x] **S4. /over-kobeon vullen (nu leeg/placeholder)**
  ...
  - **Copy:** gebruik de **herziene** Over-Kobeon-tekst uit `docs/content-werken-bij.md` (toon = expertise + samen, niet "klein team"; mét kernwaarden) — aangevuld met credentials/klanten/team/vestiging uit `docs/content-subpaginas.md`. (S6 dekt over-kobeon + werken-bij samen af.)
...
- [x] **S6. Over Kobeon & Werken bij herzien + vacatures, afstudeeropdrachten & stage** — Gebruik `docs/content-werken-bij.md`.
  - **/over-kobeon & /werken-bij:** herzie de toon naar **expertise + samen** (niet "klein team"); verwerk de **kernwaarden** (voorstel — laat Kobeon bevestigen). Hero en cultuurblok zoals in het doc.
  - **Vacatures in vast format:** intro · "Herken jij jezelf?" · "Werken zoals jij wil" · "Wat betekent dit voor jou?" · Solliciteer-CTA. Werk de bestaande titels uit met de copy uit het doc (Medior Mendix Consultant, Sales Growth Lead, Mendix Developer, AI/Agentic Engineer, UX/Prototyper, Traineeship, Open sollicitatie).
  - **Afstudeeropdrachten & stage (nieuw):** Afstudeeropdracht IT/Software, Afstudeeropdracht Commerciële Economie, en Marketing Stagiair(e) — met het afstudeer/stage-format zodat ze up-to-date te houden zijn.
  - **Data:** breid `vacancies` in `lib/siteConfig.ts` uit met de format-velden; voeg filtercategorie **"Afstuderen/Stage"** toe.
  Klaar wanneer: over-kobeon + werken-bij herzien (expertise/samen/kernwaarden), alle vacatures in het format, afstudeer-/stageplekken + marketing-stagiair toegevoegd, `npm run build` slaagt.
...
## VOORTGANGSLOG
- 2026-06-12 — EPSA-case als video gewired (EPSA Connect): `/visuals/case-epsa.mp4`/`.webm` + poster, net als HomeZero. Nieuwe client-component `CaseVideo` respecteert `prefers-reduced-motion` (toont dan de poster i.p.v. autoplay) en wordt nu door `CaseMedia` gebruikt voor álle case-video's. EPSA-copy aangescherpt met platformnaam + kernfeatures (onboarding/iDIN/IBAN/KvK/BTW, opdrachten, uren → maand afsluiten/factuur, matching, support, nieuws). screens.config epsa → video/live.
- 2026-06-12 — V10 afgerond: nieuwe `CaseMedia`-component (video → screenshot → MiniMock-fallback) gerenderd in `DeviceFrame` op `ProjectPage` én `ProjectCard`. `Project`-type kreeg optionele `video`/`images`. HomeZero-case herzien (verduurzamingsplatform) en de gecomprimeerde productvideo (`/visuals/case-homezero.*`, autoplay/muted/loop/playsinline + poster) gewired — toont nu als loopende video op de casepagina en -kaart. EPSA-case herzien naar staffing/detachering (portaal + mobiele app: onboarding/verificatie, opdrachten, uren, matching); screenshots nog niet aangeleverd → MiniMock-fallback. `screens.config.json` bijgewerkt (homezero = video/live/anonymized, epsa = verwachte screenshots/anonymized). Build slaagt.
- 2026-06-12 — S6 + V9 afgerond. S6: /over-kobeon & /werken-bij herzien naar expertise + samen (nieuwe hero's, kernwaarden-blok met "Wees geen engnek · Samen is leuker · Feedback is een cadeautje" op beide pagina's, cultuurblok geüpdatet); alle vacatures in vast format (intro · profiel · rol · voorwaarden) met de copy uit content-werken-bij.md (o.a. Medior Mendix Consultant, Sales Growth Lead); afstudeeropdrachten IT + Commerciële Economie en Marketing Stagiair(e) toegevoegd; filtercategorie "Afstuderen/Stage"; `VacancyPage` rendert het format met stage-labels. V9: `HubspotMeetings`-component (div + next/script afterInteractive) op /discovery-sessie i.p.v. Calendly-placeholder, "Wat bereid je voor?"-blok + pijnpunten-CTA naar sjoerd.beljon@kobeon.nl; alle "via Calendly" → "online, direct in je agenda". Build slaagt.
- 2026-06-11 — Content-aanvulling: per-dienst "Voorbeelden" (3–4 use-cases uit content-subpaginas.md § S1) nu als eigen blok op de dienst-detailpagina's via `serviceVoorbeelden`-map. Team gecentraliseerd in `siteConfig` (`team`) als gedeelde bron voor /over-kobeon én /werken-bij (foto's + initialen-fallback); /werken-bij toont nu het echte team i.p.v. placeholders en linkt door naar /over-kobeon. Telling bijgewerkt naar 8 (credentials + werken-bij-chip + "Wij zijn met {n}").
...
- 2026-06-12 — Echte case-assets: HomeZero-productvideo gecomprimeerd (6,3MB → mp4 0,59MB + webm 0,88MB + poster) en in `/public/visuals/` gezet. Herziene case-teksten voor HomeZero (verduurzamingsplatform) en EPSA (staffing/detachering: onboarding + opdrachten + uren + matching) toegevoegd aan content-subpaginas. Stap V10 toegevoegd om beide te wiren (video op HomeZero-case/hero; EPSA-screenshots zodra aangeleverd).
- 2026-06-01 — Kernwaarden vervangen door de échte uit het VTO (No-nonsense/"wees geen engnek", Samen is leuker, Feedback is een cadeautje), outward in de nieuwe tone-of-voice — in content-werken-bij.md én Propositie.md. Discovery-voorbereiding ("wat bereid je voor?" + pijnpunten vooraf mailen) en een warming-up-mail/agenda-omschrijving toegevoegd (HubSpot-workflow). V9 uitgebreid met het voorbereidingsblok.
- 2026-06-01 — Nieuwe doc `docs/content-werken-bij.md`: herziene Over-Kobeon & Werken-bij (expertise + samen i.p.v. "klein team"), kernwaarden-voorstel, vacature-format + uitgewerkte vacatures, afstudeeropdrachten (IT + CE) + Marketing Stagiair met format, en HubSpot-meetings-embed + agenda-omschrijving. Stappen S6 (werken bij/over/vacatures/afstudeer) en V9 (HubSpot i.p.v. Calendly) toegevoegd.
