...
- Secties via `<Section tone=...>`. Herbruikbare visuals in `/components` (DeviceFrame, MockDashboard, FloatingMetric, StatCounter, SolutionCard). Content/copy-constanten in `lib/siteConfig.ts`.
...
## Wat al staat (foundation)
Homepage met Hero, LogoStrip, Solutions ("Vier oplossingen, één platform"), Stats, FAQ, FinalCTA. Nav + Footer + SEO-basis. Placeholder-pagina's voor de overige routes.
